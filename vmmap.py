...
def get_memory_map() -> MemoryMap:
    if bool(vmmap_cache_param):
        ...
        if _persistent_memory_map is None:
            ...
            # OLD: return _persistent_memory_map directly here
            # NEW: label TLS before returning so vmmap shows [tls] instead of [anon_...]
...
def _stop_cached_memory_map() -> MemoryMap:
    # OLD: return _refine_memory_map(pwndbg.dbg.selected_inferior().vmmap())
    # NEW: label TLS region before returning so vmmap shows [tls] instead of [anon_...]
